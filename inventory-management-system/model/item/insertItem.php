	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if(filter_var($quantity, FILTER_VALIDATE_INT) === 0 || filter_var($quantity, FILTER_VALIDATE_INT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button> እባክዎን ለብዛት ትክክለኛ ቁጥር ያስገቡ</div>';
				exit();
			}
			...
			if(filter_var($unitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($unitPrice, FILTER_VALIDATE_FLOAT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>እባክዎን ለአንዱ ዋጋ ትክክለኛ ቁጥር ያስገቡ</div>';
				exit();
			}
			...
			if(!empty($discount)){
				if(filter_var($discount, FILTER_VALIDATE_FLOAT) === false){
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>እባክዎን ትክክለኛ የቅናሽ መጠን ያስገቡ</div>';
					exit();
				}
			}
			...
			if($stockStatement->rowCount() > 0){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>እቃው ማህደር ላይ ቀደም ብሎ ይገኛል እባክዎን የ <strong>አዘምን</strong> ቁልፉን ይጫኑ ወይም የተለየ የእቃ ቁጥር ይስጡ </div>';
				exit();
			} else {
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>እቃው ወደ ማህደር ገብትዋል</div>';
				exit();
			}
