	...
	if(isset($_POST['purchaseDetailsItemNumber'])){
		...
		if(isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsItemName) && isset($purchaseDetailsQuantity) && isset($purchaseDetailsUnitPrice)){
			...
			if($purchaseDetailsItemNumber == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>እባክዎን የእቃውን ቁጥር ያስገቡ</div>';
				exit();
			}
			
			
			if($purchaseDetailsItemName == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>እባክዎን የእቃውን ስም ያስገቡ</div>';
				exit();
			}
			
		
			if($purchaseDetailsQuantity == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>እባክዎን የእቃውን ብዛት ያስገቡ</div>';
				exit();
			}
			...
			if(filter_var($purchaseDetailsUnitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($purchaseDetailsUnitPrice, FILTER_VALIDATE_FLOAT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>እባክዎን የአንዱን እቃ ዋጋ በትክክል ያስገቡ</div>';
				exit();
			}
