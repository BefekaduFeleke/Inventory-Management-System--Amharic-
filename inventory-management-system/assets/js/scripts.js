...
$(document).ready(function(){
	...
	$('#deleteItem').on('click', function(){
		...
		bootbox.confirm('እርግጠኛ ነህ?', function(result){
			if(result){
				deleteItem();
			}
		});
	...
	$(tableContainerDivID).load(tableCreatorFileUrl, function(){
		...
		$(tableID).DataTable({
			...
			buttons: [
				...
				{extend: 'pdf', footer: true, orientation: 'landscape', pageSize: 'LEGAL', title: 'የግዢ ሪፖርት'},
			...
			"footerCallback": function ( row, data, start, end, display ) {
				...
				$( api.column( 6 ).footer() ).html(quantityTotal+fullPriceFilteredTotal);
				$( api.column( 7 ).footer() ).html( unitPriceTotal );
				$( api.column( 8 ).footer() ).html(fullPriceTotal );
			}
		});
	});
}
	...
	$(tableContainerDivID).load(tableCreatorFileUrl, function(){
		...
		$(tableID).DataTable({
			...
			buttons: [
				...
				{extend: 'pdf', footer: true, orientation: 'landscape', pageSize: 'LEGAL', title: 'የሽያጭ ሪፖርት'}
			...
			"footerCallback": function ( row, data, start, end, display ) {
				...
				// Update footer columns
				$( api.column( 7 ).footer() ).html(quantityFilteredTotal );
				$( api.column( 9 ).footer() ).html(fullPriceFilteredTotal +' Br');
				$( api.column( 8 ).footer() ).html(unitPriceFilteredTotal +' Br');
			}
		});
	});
}
		...
		complete: function(){
			...
			$('#' + tableID).DataTable({
				...
				buttons: [
					...
					{extend: 'pdf', footer: true, orientation: 'landscape', pageSize: 'LEGAL', title: 'አጠቃላይ የሽያጭ ሪፖርት '}
		...
		complete: function(){
			...
			$('#' + tableID).DataTable({
				...
				"footerCallback": function ( row, data, start, end, display ) {
					...
					// Update footer columns
					$( api.column( 6 ).footer() ).html(quantityFilteredTotal );
					$( api.column( 7 ).footer() ).html(unitPriceFilteredTotal +'Br');
					$( api.column( 8 ).footer() ).html(fullPriceFilteredTotal +'Br');
				}
			});
		}
	});
